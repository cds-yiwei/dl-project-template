...
The current backend uses focused tests for:

- session-first user resolution in `get_current_user` and `get_optional_user`
- OIDC callback user synchronization
- local password login gating
- session-aware logout, including explicit cookie expiry and session-store invalidation
- route-level Casbin decorators with dependency overrides

For session-backed auth tests, prefer one of these two patterns:

- Unit tests that call the route function directly and assert `request.session.clear()` side effects plus cookie expiry headers on the `Response` object.
- Middleware-level tests that mount the route inside a test app using `starsessions.InMemoryStore` or a small custom store, then assert the session record is removed after `POST /logout`.

That second pattern is the most reliable way to verify Redis-backed invalidation semantics without requiring a real Redis server in every test run.

For decorator-based routes, override the exact provider objects used by the route decorators. Example pattern:

```python
app.dependency_overrides[get_current_user] = lambda: {"username": "member", "is_superuser": False}
app.dependency_overrides[database_enforcer_provider] = lambda: enforcer
app.dependency_overrides[async_get_db] = lambda: Mock()
```

Then override the route's service dependency or patch the service method behind the route and assert on the HTTP response from `TestClient`.
