...
- **[Endpoints](endpoints.md)** - Create thin endpoints with authentication, validation, and explicit service dependencies
...
- **Application Integration** - Use the boilerplate's service layer, CRUD layer, and schemas
...
async def get_users(
    ...
    return await user_service.list_users(db=db, page=page, items_per_page=items_per_page)
...
async def create_user(user_data: UserCreate):  # ← Validates input
    return await user_service.create_user(db=db, user=user_data)  # ← Validates output
```
...
- **Maintainable** - Clear separation of HTTP concerns, business logic, and data access  
