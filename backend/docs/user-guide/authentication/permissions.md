...
## Casbin Resources In This Template

The backend uses Casbin resource-action pairs for admin-heavy operations. In the current minimal RBAC setup, the important resources are:

- `roles`: manage role definitions through `/api/v1/role` and `/api/v1/roles`
- `users_admin`: assign roles to users, manage user tiers, inspect user rate limits, and perform destructive user admin actions
- `tiers`: manage tier definitions
- `rate_limits`: manage tier rate limits

The built-in `admin` role is seeded with read and write access to all of these resources. If you upgrade an existing database, run the latest Alembic migrations before testing role APIs so the `roles` policies are backfilled.
