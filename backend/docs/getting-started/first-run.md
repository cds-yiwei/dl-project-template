...
Create `src/app/services/item_service.py`:
...
class ItemService:
  async def create_item(self, db, item):
    return await crud_items.create(db=db, object=item)

  async def get_item(self, db, item_id: int):
    return await crud_items.get(db=db, id=item_id)

  async def list_items(self, db, skip: int = 0, limit: int = 100):
    result = await crud_items.get_multi(db=db, offset=skip, limit=limit)
    return result["data"]

  async def update_item(self, db, item_id: int, item_update):
    return await crud_items.update(db=db, object=item_update, id=item_id)

  async def delete_item(self, db, item_id: int):
    await crud_items.delete(db=db, id=item_id)
    return {"message": "Item deleted successfully"}
```

Add a provider in `src/app/api/dependencies.py`:
...
async def delete_item(
  ...
  return await item_service.delete_item(db, item_id)
```
...
2. **[Database Guide](../user-guide/database/index.md)** - Learn about models, schemas, services, and CRUD operations
