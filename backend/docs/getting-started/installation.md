...
1. **Install Python dependencies**:
   ...
   This creates `backend/.venv` and installs the runtime plus development tools used by the local workflow.
...
1. **Debug from VS Code**:

   Use the workspace launch configuration at `.vscode/launch.json` and run `Backend: FastAPI (uvicorn)`.
