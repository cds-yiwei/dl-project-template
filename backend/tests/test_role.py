...
class TestReadRoles:
    ...
    async def test_read_roles_success(self, mock_db):
        mock_service = Mock()
        mock_service.list_roles = AsyncMock(return_value={"data": [{"id": 1, "name": "admin"}]})

        result = await unwrap_endpoint(read_roles)(Mock(), mock_db, mock_service, page=1, items_per_page=10)

        assert result == {"data": [{"id": 1, "name": "admin"}]}
    ...
    async def test_read_role_delegates_to_service(self, mock_db):
        mock_service = Mock()
        mock_service.get_role_by_name = AsyncMock(return_value={"id": 1, "name": "editor"})

        result = await unwrap_endpoint(read_role)(Mock(), "editor", mock_db, mock_service)

        assert result == {"id": 1, "name": "editor"}
        mock_service.get_role_by_name.assert_awaited_once_with(db=mock_db, name="editor")
...
class TestEraseRole:
    ...
    async def test_erase_role_soft_deletes_role(self, mock_db):
        ...
        result = await unwrap_endpoint(erase_role)(Mock(), "editor", mock_db, mock_service)

        assert result == {"message": "Role deleted"}
        mock_service.delete_role.assert_awaited_once_with(db=mock_db, name="editor")