...
class TestCasbinSubjectProvider:
    ...
    async def test_get_casbin_subject_returns_username_for_regular_user(self):
        subject = await get_casbin_subject({"username": "member", "is_superuser": False, "role_id": None}, None)

        assert subject == "member"

    @pytest.mark.asyncio
    async def test_get_casbin_subject_returns_role_name_when_user_has_role(self, mock_db):
        mock_db.execute = AsyncMock(return_value=Mock(scalar_one_or_none=Mock(return_value="editor")))

        subject = await get_casbin_subject({"username": "member", "is_superuser": False, "role_id": 7}, mock_db)
